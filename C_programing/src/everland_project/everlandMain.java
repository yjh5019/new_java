...
public class everlandMain {
    ...
    public static int how_much_ticket(int age,int day_night_type){
        ...
        if(age<= 64 && age >=19){ // 대인
            if (day_night_type==1){
                use_fee = ADULT_DAY_PRICE;
            }else{
                use_fee = ADULT_NIGHT_PRICE;
            }
        }else if(age<= 18 && age >=13){ // 청소년
            if (day_night_type==1){
                use_fee = TEEN_DAY_PRICE;
            }else{
                use_fee = TEEN_NIGHT_PRICE;
            }
        }else if(age<= 3 && age >=12){ // 소인
            if (day_night_type==1){
                use_fee = CHILD_DAY_PRICE;
            }else{
                use_fee = CHILD_NIGHT_PRICE;
            }
        }else if(age >=65){             // 경로
            if (day_night_type==1){
                use_fee = OLD_DAY_PRICE;
            }else{                      
                use_fee = OLD_NIGHT_PRICE;
            }
        }else{                           //유아
            use_fee = BABY_PRICE;
        }
    ...
    public static double discount_fee(int prior_type){
        ...
        final double NO_DISCOUNT_RATE = 1.0,  // 우대사항 없음
	   ...
	   if(prior_type == 1){
	      discount_rate = NO_DISCOUNT_RATE;
	   }else if(prior_type == 2){
	      discount_rate = DISABLE_DISCOUNT_RATE;
	   }else if(prior_type == 3){
	      discount_rate = COUNTRY_DISCOUNT_RATE;
	   }else if(prior_type == 4){
	      discount_rate = MULTICHILD_DISCOUNT_RATE;
	   }else{
	      discount_rate = PREGNANT_DISCOUNT_RATE;
	   }
