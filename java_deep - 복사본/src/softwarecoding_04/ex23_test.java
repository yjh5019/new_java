...
public class ex23 {
	...
	public static void main(String[] args) {
		...
		int k37_iNumVal = 1000000000;
//		int k37_iNumVal = 1001034567;
		...
		while(true) {
			...
			if(k37_sNumVal.substring(k37_i,k37_i+1).equals("0")) {
				if(unitX[k37_j].equals("만")||unitX[k37_j].equals("억")) { // unitX 배열에서 j번째 한글 단어가 "만"이나 "억"과 같으면
					k37_sNumVoice = k37_sNumVoice+""+unitX[k37_j]; // sNumVoice 문자열에 공백 문자열을 추가함
			...
			//이 부분은 이해가 잘....
			// sNumVal 문자열에서 i+1번째 문자 하나를 추출하여 units 배열에서 해당하는 한글 단어를 가져와 sNumVoice 문자열에 추가(????????)
			// 이어서 j번째 한글 단어를 unitX 배열에서 가져와 sNumVoice 문자열에 추가함
			else {
		...
		System.out.println(k37_sNumVoice.length());
	}
